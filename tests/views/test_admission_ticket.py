...
def test_email_cannot_be_invalid(token_client, event):
    resp = token_client.post(
        "/api/v1/admission-tickets/attendee-ticket/",
        data={
            'attendee_email': 'SELECT id FROM sushi; --',
            'events': []
        },
        format='json')
...
def test_email_has_tickets_with_multiple_events(token_client, event, event2, order_event_2):
    resp = token_client.post(
        "/api/v1/admission-tickets/attendee-ticket/",
        data={
            'attendee_email': 'test@email.it',
